...
internal class HydraAdminRepoImplTest {
    ...
    @Container
    val mockServer = MockServerContainer(DockerImageName
    ...
    fun `POSITIVE ~~ getLoginRequest`() {
        MockServerClient(mockServer.host, mockServer.serverPort).use { mockClient ->
            ...
            val expectations = mockClient.`when`(request("/oauth2/auth/requests/login").apply {
                withMethod("GET")
    ...
    fun `POSITIVE ~~ acceptLogin`() {
        MockServerClient(mockServer.host, mockServer.serverPort).use { mockClient ->
            ...
            val expectations = mockClient.`when`(request("/oauth2/auth/requests/login/accept").apply {
                withMethod("PUT")
                withQueryStringParameter("login_challenge", loginChallenge)
                withContentType(MediaType.APPLICATION_JSON_UTF_8)
                withBody(json(AcceptLoginRequest().apply {
                    subject(subject)
                }))
            }).respond(response().apply {
    ...
    fun `POSITIVE ~~ getConsent`() {
        MockServerClient(mockServer.host, mockServer.serverPort).use { mockClient ->
            ...
            val expectations = mockClient.`when`(request("/oauth2/auth/requests/consent").apply {
                withMethod("GET")
    ...
    fun `POSITIVE ~~ acceptConsent`() {
        MockServerClient(mockServer.host, mockServer.serverPort).use { mockClient ->
            ...
            val expectations = mockClient.`when`(request("/oauth2/auth/requests/consent/accept").apply {
                withMethod("PUT")
                withQueryStringParameter("consent_challenge", consentChallenge)
                withContentType(MediaType.APPLICATION_JSON_UTF_8)
